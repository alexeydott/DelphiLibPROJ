...
type
  ...
  EWKTError = class(EArgumentException);
    ...
    function ItemsByKey(const AKey: string; Dest: TList): Integer;
    ...
    class function NewFromString(const ADefn: string): TWKTCRSDefinition;
	...
	protected
        ...
        function GetProjectionParameters: TWktNodeList;
	...
	public
        ...
        property ProjectionParameters: TWktNodeList read GetProjectionParameters;
...
function TWktNode.GetAttributeName: string;
begin
	...
	begin
		case CurrentChar of
			WKT_BRACKET_OPEN:
              ...
              if SameText('PROJCS', string(TempValue)) then
							  Child := TWKTProjectedCRS.Create(Self)
              else
              if SameText('GEOGCS',string(TempValue)) then
                Child := TWKTGeoCRS.Create(Self)
              else
...
function TWktNodeList.ItemsByKey(const AKey: string; Dest: TList): Integer;
var
  I: Integer;
  N: TWktNode;
begin
  Dest.Clear;
	...
	begin
		N := Items[I];
		if SameText(N.Keyword, AKey) then
      Dest.Add(N);
...
class function TWKTCRSDefinition.NewFromString(const ADefn: string): TWKTCRSDefinition;
begin
  Result := nil;

  if ADefn.Trim.StartsWith(WKT_KWD_PROJCS+WKT_BRACKET_OPEN) then
    Result := TWKTProjectedCRS.Create(nil)
  else
  if ADefn.Trim.StartsWith(WKT_KWD_GEOGCS+WKT_BRACKET_OPEN) then
    Result := TWKTGeoCRS.Create(nil);

  if Assigned(Result) then
    Result.LoadFromString(ADefn);
end;
...
destructor TWKTProjectedCRS.Destroy;
begin
...
begin
	if FGeoCS = nil then
	begin
    if not Find('GEOGCS',n) then
    begin
		  n := TWKTGeoCRS.Create(Self);
  		Add(n);
    end;
    FGeoCS := TWKTGeoCRS(n);
	end;
...
function TWKTProjectedCRS.GetProjectionParameters: TWktNodeList;
begin
  if FParameters = nil then
  begin
    FParameters := TWktNodeList.Create(False);
    ItemsByKey(WKT_KWD_PARAMETER,FParameters);
  end;
...
begin
	...
	begin
		if not DatumNode.Find(WKT_KWD_TOWGS84,FToWGS) then
			...
			Result:= FToWGS.Attributes.CommaText
	end
	else
...
begin
	...
	begin
		...
		begin
			FSpheroid := TryGetDatumNode.AddChild(WKT_ALT_KWD_ELLIPSOID, WKT_KWD_ELLIPSOID_WGS84,WKT_KWD_ELLIPSOID_WGS84_AF);
		end;
	end;
